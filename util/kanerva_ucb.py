...
class BaseKanervaCoder:
    def __init__(self, observation_space: gym.spaces.Space, n_prototypes: int, n_closest: int, seed: int = 0, c: float = 1.0):
        """
    ...
    def get_features(self, data: np.ndarray) -> np.ndarray:
        ...
        indexes = np.argpartition(dist, self.n_closest)[:self.n_closest]
        self.visit_counts[indexes] += 1
        return indexes

    def get_features_UBC(self, data: np.ndarray) -> np.ndarray:
        """
        Gets the active features for the input data using UCB action selection mechanism.
        Updates the visit counts

        :param data: input data
        :return: array of active feature indexes
        """
        dist = self.distance(data)
        ucb_values = dist + self.c * np.sqrt(np.log(np.sum(self.visit_counts)) / (self.visit_counts + 1e-5))
        indexes = np.argpartition(ucb_values, self.n_closest)[:self.n_closest]
        self.visit_counts[indexes] += 1
        return indexes
